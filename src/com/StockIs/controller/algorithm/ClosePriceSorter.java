 ...
 * LMU Id: 23048683
 */
public class ClosePriceSorter {
    private List<stockModel> stockSortList;

    // Constructor
    public ClosePriceSorter() {
    ...
    private int findExtremumIndexByOpenPrice(List<stockModel> stockSortList, int startIndex, boolean isDesc) {
        ...
        for (int j = startIndex + 1; j < stockSortList.size(); j++) {
            if (shouldSwapByOpenPrice(stockSortList.get(j).getClosePrice(), stockSortList.get(extremumIndex).getClosePrice(), isDesc)) {
                extremumIndex = j;
            }
        }
