...
/**
 * A class that sorts a list of stockModel objects by their Stock ID.
 * 
 * @author prashantrijal
 * LMU Id: 23048683
 */
public class SortStockId {
    ...
    /**
     * Sorts a list of stockModel objects by their Stock ID in ascending or descending order.
     *
    ...
    private int findExtremumIndex(List<stockModel> stockSortList, int startIndex, boolean isDesc) {
        ...
        for (int j = startIndex + 1; j < stockSortList.size(); j++) {
            if (shouldSwap(stockSortList.get(j).getStockId(), stockSortList.get(extremumIndex).getStockId(), isDesc)) {
                extremumIndex = j;
            }
        }
     ...
     *
     * @param current the current Stock ID
     * @param extremum the current extremum Stock ID
     * @param isDesc specifies the sort order (true for descending, false for ascending)
